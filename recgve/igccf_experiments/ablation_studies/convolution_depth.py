...
if __name__ == "__main__":
    ...
    for project in PROJECTS:
        ...
        for cd in CONV_DEPTH:
            ...
            run_parameters_dict["convolution_depth"] = cd
            run = wandb.init(project="ablation_{}".format(project), config=run_parameters_dict)
            run.name = f"igccf_conv_depth_{cd}"
            model = IGCCF(
                train_df,
                embeddings_size=run_parameters_dict["embedding_size"],
                convolution_depth=cd,
            ...
            es_handler = EarlyStoppingHandlerTensorFlow(
                patience=100, save_path=os.path.join(run.dir, "best_models"),
            )
            ...
            wandb.log({"epoch_best_result": summary["epoch_best_result"]})
            wandb.log(test_evaluator.result_dict)

            es_handler.update(
                summary["epoch_best_result"],
                test_evaluator.result_dict["Recall@20"],
                "Recall@20",
                model,
            )
            run.finish()
